 ...
 * (signed-out) store's current data. Mocking `registerSweepTarget` here
 * (rather than calling the real one) lets us capture the exact target
 * object the store registers, without touching `importSweep.io.ts` itself
...
describe('import-sweep registration', () => {
  it("registers a sweep target with the correct toolId and getLocalItems() reflecting the local store's current data", () => {
    ...
    expect(target.toolId).toBe('color-converter')

    useColorConverterStore.setState({
      inputValue: '#ff0000',
      sliderMode: 'rgb',
      history: ['#ff0000'],
    })
