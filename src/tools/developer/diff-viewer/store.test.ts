...
describe('import-sweep registration', () => {
  it("registers a sweep target with the correct toolId and getLocalItems() reflecting the local store's current data", () => {
    ...
    expect(target.toolId).toBe('diff-viewer')

    useDiffViewerStore.setState({ original: 'a', modified: 'b', viewMode: 'side-by-side' })

    const items = target.getLocalItems()
    expect(items).toEqual([
      {
        itemId: 'default',
        data: expect.objectContaining({ original: 'a', modified: 'b', viewMode: 'side-by-side' }),
      },
    ])
  })
})
