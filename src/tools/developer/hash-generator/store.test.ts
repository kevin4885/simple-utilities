...
describe('import-sweep registration', () => {
  it("registers a sweep target with the correct toolId and getLocalItems() reflecting the local store's current data", () => {
    ...
    expect(target.toolId).toBe('hash-generator')

    useHashGeneratorStore.setState({ inputText: 'hello', hexCase: 'upper', outputEncoding: 'base64' })

    const items = target.getLocalItems()
    expect(items).toEqual([
      {
        itemId: 'default',
        data: expect.objectContaining({ inputText: 'hello', hexCase: 'upper', outputEncoding: 'base64' }),
      },
    ])
  })
})
