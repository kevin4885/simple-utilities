/**
 * timestamp-converter/store.test.ts
 *
...
describe('import-sweep registration', () => {
  it("registers a sweep target with the correct toolId and getLocalItems() reflecting the local store's current data", () => {
    ...
    expect(target.toolId).toBe('timestamp-converter')

    useTimestampConverterStore.setState({ timestampInput: '1700000000', unitOverride: 'millis' })

    const items = target.getLocalItems()
    expect(items).toEqual([
      {
        itemId: 'default',
        data: expect.objectContaining({ timestampInput: '1700000000', unitOverride: 'millis' }),
      },
    ])
  })
})
