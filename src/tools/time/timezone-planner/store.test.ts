/**
 * timezone-planner/store.test.ts
 *
...
import { useTimezonePlannerStore } from './store'
...
describe('import-sweep registration', () => {
  it("registers a sweep target with the correct toolId and getLocalItems() reflecting the local store's current data", () => {
    ...
    expect(target.toolId).toBe('timezone-planner')

    const zones = [{ zone: 'America/New_York', label: 'New York', isLocal: false }]
    useTimezonePlannerStore.setState({ zones, selectedDate: '2024-01-01' })

    const items = target.getLocalItems()
    expect(items).toEqual([
      {
        itemId: 'default',
        data: expect.objectContaining({ zones, selectedDate: '2024-01-01' }),
      },
    ])
  })
})
